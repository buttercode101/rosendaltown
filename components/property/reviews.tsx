  ...
  return (
    <section className="container-editorial py-16">
      ...
      <div className="mt-8 grid gap-5 md:grid-cols-3">
        {REVIEWS.map((r) => (
          <figure key={r.name} className="rounded-2xl border border-cream/10 bg-forest-mist p-7">
            ...
            <figcaption className="mt-5 text-sm text-cream/55">{r.name} · {r.from}</figcaption>
          </figure>
        ))}
      </div>
    </section>
