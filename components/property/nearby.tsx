...
export function NearbyMap() {
  return (
    <section className="bg-forest-mist py-16">
      <div className="container-editorial">
        <p className="eyebrow">Around you</p>
        <h2 className="mt-3 font-serif text-3xl md:text-4xl">A short walk from your door.</h2>
        <div className="mt-8 grid gap-px overflow-hidden rounded-2xl bg-cream/10 md:grid-cols-3">
          {NEARBY.map((n) => (
            <div key={n.label} className="flex items-center justify-between bg-forest-deep px-7 py-6">
              <span className="font-serif text-xl">{n.label}</span>
              <span className="text-sm text-cream/55">{n.mins} min</span>
            </div>
          ))}
        </div>
      </div>
